...
<body>
   <main class="h-dvh flex justify-center items-center bg-linear-to-b from-0% from-[#004d42] to-70% to-[#ebab21]">
      <form method="POST" action="{{ route('log-in') }}" class="bg-gray-100 flex flex-col p-3.5 w-96 rounded-2xl shadow-xl">
         @csrf
         <h1 class="text-3xl font-bold self-center m-3">Log in</h1>
         @error('credentials')
         ...
         <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-1.5">
               <input id="remember" type="checkbox" name="remember" class="w-4 h-4 accent-[#004d42]">
               <label for="remember" class="text-xs">Mantener sesión</label>
            </div>
            <div class="">
               <p class="text-xs">¿No tienes cuenta? <a href="{{ route('register') }}" class="text-[#004d42] underline">Regístrate</a></p>
            </div>
         </div>
         <button type="submit" class="bg-[#ebab21] outline-2 outline-[#ebab21] py-4 rounded-4xl text-black font-bold mb-4 cursor-pointer">Acceder</button>
      </form>
   </main>
</body>
</html>
